...
class VarianceLoss(ReferenceFrame):
    ...
    def episode_loss(self, log_history):
        ...
        for d_t in [log_history[-1][key] for key in self.loss_fields]:
            d_t = Log.dict_to_list(d_t)
            d_std = np.std(d_t, axis=0)
            var_loss += np.sum(np.maximum(self.gamma - d_std, 0))

        return var_loss * self.loss_factor
